...
class TrackedRouteScene(object):
    ...
    def tracked_route(self, count):
        ...
        eta = tracked.get("time_estimated_arrival")
        real_dep = tracked.get("time_real_departure")
        # Discard ETA if it predates the actual departure — stale API data
        if eta and real_dep and eta < real_dep:
            eta = None
